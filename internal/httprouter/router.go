...
type Router struct {
	...
	GlobalOPTIONS caddyhttp.Handler
	...
	// is called.
	MethodNotAllowed caddyhttp.Handler
...
// Handler is an adapter which allows the usage of a caddyhttp.Handler as a
// request handle.
// The Params are available in the request context under ParamsKey.
func (r *Router) Handler(method, path string, handler caddyhttp.Handler) {
...
// request handle.
func (r *Router) HandlerFunc(method, path string, handler caddyhttp.HandlerFunc) {
...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) (err error) {
	...
	if req.Method == http.MethodOptions && r.HandleOPTIONS {
		...
		if allow := r.allowed(path, http.MethodOptions); allow != "" {
			...
			if r.GlobalOPTIONS != nil {
				err = r.GlobalOPTIONS.ServeHTTP(w, req)
			}
			return
		}
		...
		if allow := r.allowed(path, req.Method); allow != "" {
			...
			if r.MethodNotAllowed != nil {
				err = r.MethodNotAllowed.ServeHTTP(w, req)
			} else {
				http.Error(w,
	...
	// Handle 404
	if r.NotFound != nil {
		err = r.NotFound.ServeHTTP(w, req)
	} else {
		http.NotFound(w, req)
	}
